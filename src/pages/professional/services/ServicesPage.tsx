...
import { useMyServices, useCreateService, useUpdateService, useMyServicesT } from "@/hooks/useServices";
...
export const ServicesPage = () => {
  const { data: services = [], isLoading } = useMyServicesT();
  ...
  const startEdit = (s: Service) => {
    ...
    reset({
      ...
      bufferMinutes: s.bufferMinutes ?? undefined,
    });
  };

  // Toggle activo/inactivo — solo actualiza el campo isActive
    ...
    <div className="page">
      ...
      {showForm && (
        <div className="card mb-6">
          <div className="card-header">
            <span className="card-title">{editing ? "Editar servicio" : "Nuevo servicio"}</span>
          </div>
          <div className="card-body">
            <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="form-label">Nombre del servicio *</label>
                <input {...register("name", { required: "Requerido" })} className="form-input"
      ...
      <div className="card">
        {services.length === 0 ? (
          ...
          services.map((s) => (
            <div key={s.id}
              className={`flex items-center gap-4 px-5 py-4 border-b border-gray-100 last:border-0 transition-opacity ${
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-400 mt-0.5">
                  ⏱ {s.durationMinutes} min{s.bufferMinutes ? ` + ${s.bufferMinutes}m buffer` : ""}
                </div>
                {s.description && (
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  className={`btn btn-xs disabled:opacity-50 disabled:cursor-not-allowed ${
                    s.isActive ? "btn-outline text-amber-600 hover:bg-amber-50 hover:border-amber-300"
                               : "btn-success"
                  }`}
                  title={s.isActive ? "Desactivar — no aparece en nuevas reservas" : "Activar"}>
      ...
      {services.some(s => !s.isActive) && (
        <p className="text-xs text-gray-400 mt-3 text-center">
          Los servicios inactivos no aparecen al reservar una cita
        </p>
      )}
    </div>
  );
